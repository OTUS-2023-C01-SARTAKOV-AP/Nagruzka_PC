// здесь учет , описание всех ошибок, которые есть в программе (таблица ошибок)
// передача записи об ошибке в другие функции, чтобы она мога сделать запись в лог файл
// функция по возврату кода ошибки как текст
...
int f_текст_ошибки(char* ars_сообщение, int номер_ошибки)
{
    ...
    {
        switch (x>0){
            case 1: strcat(ars_текст_ошибки, ars_сообщение); 
                    strcat(ars_текст_ошибки, " Ошибка № >= 500. Зарезервированны На мои программы = сокет, многопоточность и прочие.\n\t\t"); 
                    break;    // strcat()
        }
        //ошибка = 0;
    }
    ...
    if (x>-500 and x<=-1)
    {
        switch (x){
            ...
            case -20:   strcat(ars_текст_ошибки, " [-20]. Ошибки работы с БД PostgreSQL. \t");      // запросы_к_бд.c 
                        strcat(ars_текст_ошибки, ars_сообщение);
    ...
    f_логфайл_запись_сообщения(ars_текст_ошибки);
...
int f_текст_ошибки_POSIX(char* запрос_system, int номер_ошибки)
{
    // отрицательные ошибки - это самописные ошибки
    int x = номер_ошибки;
